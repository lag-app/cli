...
fn show_config() -> Result<()> {
    ...
    println!(
        ...
        if cfg.api_url.is_none() { "(default)" } else { "(custom)" }
    );
    println!(
        "  ptt-key:       {}",
    ...
    println!(
        "  vad-threshold: {}",
        (cfg.vad_threshold * 100.0) as u32
    );
    println!("\nConfig file: {}", config::config_dir().join("config.json").display());
    Ok(())
}
...
fn set_config(key: &str, value: &str) -> Result<()> {
    ...
    match key {
        ...
        _ => {
            anyhow::bail!("Unknown config key '{}'. Valid keys: api-url, ptt-key, vad-threshold", key);
        }
    }
